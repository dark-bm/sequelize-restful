      ...
      if (parseInt(identifier) === NaN || !this.isAllowed(modelName) || !this.isAllowed(associatedModelName)){
        ...
        switch(req.method) {
          ...
          case 'DELETE':
            ...
            this.findSingleAssociatedModel(modelName, identifier, associatedModelName, function(associatedModel) {
              if (!!associatedModel) {
...
Router.prototype.handleSuccess = function(data, optionsOrCallback, callback) {
  ...
  if (Array.isArray(data.rows) && (typeof data.count === "number")) {
    callback({
      status: 'success',
      count: data.count,
      limit: data.queryOptions.limit,
      offset: data.queryOptions.offset,
      where: data.queryOptions.where,
      order: data.queryOptions.order,
      data: data.rows
    }, optionsOrCallback)
  } else {
    callback({
...
var handleResourceIndex = function(modelName, query, callback) {
  ...
  var queryOptions = {
    where: (!!query && !!query.where) ? query.where : null,
    offset: (!!query && !!query.offset) ? parseInt(query.offset) : null,
    limit: (!!query && !!query.limit) ? parseInt(query.limit) : null,
    order: (!!query && !!query.order) ? [query.order] : null
  }

  
  var processIncludes = function(target) {
    if (Array.isArray(target)) return target.map(processIncludes);
    else if (typeof target === 'string') return self.findDAOFactory(target);
    else if (target.name && target.include) {
      return {
        model: processIncludes(target.name),
        include: processIncludes(target.include)
      }
    } 
  };
  ...
  if (!!query && !!query.include) {
    ...
    //  queryOptions.include = query.include.map(function (modelName) {
    //    return self.findDAOFactory(modelName)
    //  });
    //} else {
    //  queryOptions.include = [self.findDAOFactory(query.include)]
    //}
    queryOptions.include = processIncludes(query.include);
  }
    ...
    daoFactory
      ...
      .then(function(entry) {
        if (!entry){
          ...
          entry.updateAttributes(attributes).then(function(entry) {
            this.handleSuccess(entry.get(), callback)
