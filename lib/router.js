...
var handleResourceIndex = function(modelName, query, callback) {
  ...
  var processIncludes = function(target) {
    ...
    else if (typeof target === 'string') return self.findDAOFactory(target) || self.findAssociation(daoFactory, target);
    else if (target.name && (target.include || target.where)) {
      var ret = {
        model: processIncludes(target.name)
      }
      if (target.include) {
        ret.include =  processIncludes(target.include);
      }
      if (target.where) {
        ret.where = target.where;
      }
      return ret;
    } 
  };
